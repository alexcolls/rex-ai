...
def firstRun():
    ...
    try:
        ...
        if ans.__contains__("y"):
            ...
            REQ_PATH = os.path.join(ENV_PATH, "requirements.txt")
            print(ENV_PATH)
            os.system(f"pip3 install -r {REQ_PATH}")
    except:
        print(
    ...
    VARS_PATH = os.path.join(ENV_PATH, "variables.json")

    with open(VARS_PATH) as x:
