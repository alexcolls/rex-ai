...
PARAMS = os.path.exists(__file__[:-3]+'.h5')


SYMBOL = 'USD_JPY'

PARAMS = os.path.exists(__file__[:-3]+'_'+SYMBOL+'.h5')
...
def prepData ( symbol=SYMBOL, start_year=2010, final_year=2015, threshold=THRESHOLD, lookback=120 ):
...
if __name__ == "__main__":
    ...
    if not PARAMS:
        ...
        y_train, X_train = prepData(SYMBOL, TRAIN_YEAR, VALID_YEAR, THRESHOLD)
        y_test, X_test = prepData(SYMBOL, VALID_YEAR, TEST_YEAR, THRESHOLD)
        ...
        y_val, X_val = prepData(SYMBOL, VALID_YEAR, TEST_YEAR-1, THRESHOLD)
        y_test, X_test = prepData(SYMBOL, TEST_YEAR, FINAL_YEAR, THRESHOLD)

        model = models.load_model(__file__[:-3]+'_'+SYMBOL+'.h5')
