...
def correlations(df, rate=120):
    data = pd.DataFrame([])
    for sym in SYMBOLS:
        data[f"{sym[:3]}_{sym[4:]}_corr"] = (
            df[f"{sym[:3]}"].rolling(rate).corr(df[f"{sym[4:]}"])
        )
    data.index = df.index
    return data
...
def sharpe_ratio(df, window=24):
    ...
    data = pd.DataFrame([])
    for currency in df.columns:
        data[f"{currency}_sharpe"] = (
            df[currency].rolling(window).mean() / df[currency].rolling(window).std()
        )
    data.index = df.index
...
def atr(idxs: pd.DataFrame, low: pd.DataFrame, high: pd.DataFrame, window: int = 14):
    ...
    for currency in idxs.columns:
        ...
        data[f"tr1_{currency}"] = np.abs(
            data[f"high_{currency}"] - data[f"idxs_{currency}"].shift()
        )
        data[f"tr2_{currency}"] = np.abs(
            data[f"low_{currency}"] - data[f"idxs_{currency}"].shift()
        )
        data[f"tr_{currency}"] = data[
            [f"tr0_{currency}", f"tr1_{currency}", f"tr2_{currency}"]
        ].max(axis=1)
        data[f"atr_{currency}"] = (
            data[f"tr_{currency}"].ewm(alpha=1 / window, adjust=False).mean()
        )
        data.drop(
            columns=[
                f"idxs_{currency}",
...
def ema(df, window=48):
    ...
    data = pd.DataFrame([])
    for currency in df.columns:
        data[f"{currency}_ema"] = (
            df[currency].ewm(alpha=1 / window, adjust=False).mean()
        )
    data.index = df.index
