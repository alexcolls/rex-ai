...
class RiskManagement:
    ...
    def mean_volatility_prediction(self, logs, rate=120):
        ...
        for ccy in logs.columns:
            data[ccy] = (
                logs[ccy].rolling(rate).mean() + logs[ccy].rolling(rate).std() * 2
            )
    ...
    def correlation_pairs(self, pred_vols):
        ...
        for pair in pred_vols.columns:
            for second_pair in pred_vols.columns:
                data[f"{pair}_{second_pair}_corr"] = pred_vols[pair].corr(
                    pred_vols[second_pair]
                )
        ...
        for sym in pred_vol.columns:
            ...
            for sym2 in pred_vol.columns:
                ...
                if sym == sym2:
                    ...
                    real_vol += (
                        pred_vol[sym2]
                        * correlations.filter(regex=sym + "_" + sym2).values[0][0]
                    )
        ...
        for ccy in classification.columns:
            ...
            if int(classification[ccy]) != 0:
                ...
                if ccy.split("_")[0] == "USD":
                    ...
                    d["size"] = round(
                        risked
                        * float(exchange_rate[ccy.split("_")[0]])
                        * int(classification[ccy])
                    )
                    trade_dataframe["size"].append(
                        round(risked * float(exchange_rate[ccy.split("_")[0]]))
                    )
